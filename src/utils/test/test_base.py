...
from .runtime_annotations import RuntimeConfig, RuntimeContext, runtime_mock, _ast_analyzer, auto_analyze, intelligent_preset
...
class RuntimeTestCase(SmartMockTestCase):
    ...
    def apply_runtime_template(self, template_name: str, variables: Optional[Dict] = None, **overrides):
...
class ParametrizedTestCase(RuntimeTestCase):
    ...
    @classmethod
    def generate_test_cases(cls, test_name: str, parameters: List[Dict[str, Any]], 
    ...
    @classmethod
    def create_test_matrix(cls, test_name: str, parameter_matrix: Dict[str, List[Any]], 
                          test_method: Callable):
...
class SmartTestCase(unittest.TestCase):
    ...
    def apply_intelligent_preset(self, preset_name: str = None, **overrides) -> RuntimeContext:
        """
    ...
    def auto_mock_setup(self, enable_warnings: bool = True) -> RuntimeContext:
        ...
        if enable_warnings and analysis.get('complexity_score', 1) > 5:
            print(f"⚠️  High complexity test detected (score: {analysis['complexity_score']}). Consider breaking into smaller tests.")
    ...
    def get_pattern_suggestions(self) -> List[str]:
        ...
        if complexity > 3:
            suggestions.append(f"Consider breaking this test into smaller methods (complexity: {complexity})")
        
        if len(patterns) > 2:
            suggestions.append(f"Multiple patterns detected: {', '.join(patterns)}. Consider using '{recommended_preset}' preset.")
        
        if 'logging_system' in patterns and 'file_operations' in patterns:
            suggestions.append("Use @intelligent_preset('logging_system') for optimal logging + file operation mocking")
    ...
    def assert_mock_called_with_pattern(mock_obj: Mock, *args, **kwargs):
        ...
        for call in mock_obj.call_args_list:
            ...
            if (len(args) == 0 or call_args == args) and \
               (len(kwargs) == 0 or all(call_kwargs.get(k) == v for k, v in kwargs.items())):
                return True
                
        raise AssertionError(f"Mock {mock_obj} was not called with expected arguments")
        
    @staticmethod
    def assert_output_contains(captured_output: str, expected_text: str, case_sensitive: bool = True):
        ...
        if expected_text not in captured_output:
            raise AssertionError(f"Expected '{expected_text}' not found in output: '{captured_output}'")
    ...
    def assert_environment_variable(key: str, expected_value: Optional[str] = None):
        ...
        if expected_value is None:
            ...
            if actual_value != expected_value:
                raise AssertionError(f"Environment variable '{key}' expected '{expected_value}', got '{actual_value}'")
...
class CommonFixtures:
    ...
    @staticmethod
    def sample_sys_argv(program_name: str = "test_program", args: Optional[List[str]] = None):
...
__all__ = [
    # Base classes
    'BaseTestCase', 'SmartMockTestCase', 'RuntimeTestCase', 'ParametrizedTestCase',
    'PytestBase', 'StandardTestCase', 'StandardPytestBase', 'SmartTestCase',
