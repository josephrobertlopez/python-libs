...
class SmartMockContext:
    ...
    def __init__(
        self, target_path: str, auto_mock: Optional[Dict[str, Any]] = None, **kwargs
    ):
        ...
        for method_name, behavior in class_methods.items():
            if callable(behavior):
                setattr(
                    MockClass,
                    method_name,
                    lambda self, *args, **kwargs: behavior(*args, **kwargs),
                )
            else:
...
class MockBuilder:
    ...
    def add_file_mocks(self, file_contents: Optional[Dict[str, str]] = None, **kwargs):
        ...
        builtins_config = {"open": mock_open, **kwargs}
    ...
    def add_network_mocks(self, responses: Optional[Dict[str, Dict]] = None, **kwargs):
        ...
        def mock_post(url, **req_kwargs):
            return MockFactory.create_response_mock(
                **responses.get("POST", {"status_code": 201})
            )

        requests_config = {"get": mock_get, "post": mock_post, **kwargs}
...
class MockInspector:
    ...
    def inspect_context(context: SmartMockContext) -> Dict[str, Any]:
        ...
        return {
            ...
            "mock_types": {
                name: type(mock).__name__
                for name, mock in context.mock_registry.items()
            },
