...
"""
    ...
    with tempfile.NamedTemporaryFile(suffix=".zip") as build_zip:
        ...
        typer.echo(f"Uploading build artifacts to s3://{BUCKET_NAME}/{build_artifact_key}")
        get_s3_client().upload_file(
            build_zip.name, 
            BUCKET_NAME, 
            build_artifact_key,
    ...
    with tempfile.NamedTemporaryFile() as exe_file:
        ...
        typer.echo(f"Uploading final executable to s3://{BUCKET_NAME}/{final_artifact_key}")
        get_s3_client().upload_file(
            exe_file.name, 
            BUCKET_NAME, 
            final_artifact_key,
    ...
    with tempfile.NamedTemporaryFile(suffix=".json") as metadata_file:
        ...
        # Upload metadata
        typer.echo(f"Uploading pipeline metadata to s3://{BUCKET_NAME}/{pipeline_metadata_key}")
        get_s3_client().upload_file(
            metadata_file.name, 
            BUCKET_NAME, 
    ...
    for obj in response.get('Contents', []):
        ...
        try:
            obj_metadata = get_s3_client().head_object(Bucket=BUCKET_NAME, Key=obj['Key'])
