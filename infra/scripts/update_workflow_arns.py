...
def update_workflow_files(role_arn: str, workflow_dir: str) -> None:
    ...
    for file_path in workflow_files:
        ...
        if placeholder_arn not in content:
            console.print(
                f"  No placeholder ARN found in {os.path.basename(file_path)}",
                style="yellow",
            )
            continue
        ...
        console.print(
            f"  Updated {os.path.basename(file_path)} with role ARN: {role_arn}",
            style="green",
        )
...
def main(
    ...
    if not os.path.isdir(workflow_dir):
        console.print(
            f"Error: Workflow directory '{workflow_dir}' not found", style="bold red"
        )
        sys.exit(1)
    ...
    console.print(
        "\nu2705 GitHub Actions workflow files updated successfully!",
        style="bold green",
    )
