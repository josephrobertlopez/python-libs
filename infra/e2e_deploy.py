    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
        ...
        for root, dirs, files in os.walk(project_dir):
            ...
            dirs[:] = [
                d
                for d in dirs
                if d not in [".git", "__pycache__", ".pytest_cache", "dist", "build"]
            ]
...
def deploy(
    ...
    try:
        ...
        typer.echo(
            f"Bucket '{bucket_name}' created successfully with versioning enabled"
        )
    ...
    with tempfile.TemporaryDirectory() as temp_dir_str:
        ...
        with open(source_zip, "rb") as file_data:
            s3_client.put_object(Bucket=bucket_name, Key=source_key, Body=file_data)
    ...
    typer.echo(
        "This bucket is in your LocalStack environment and will persist until you stop LocalStack."
    )
...
def cleanup(bucket_name: str):
    ...
    confirm = typer.confirm(
        f"Are you sure you want to delete ALL objects in bucket '{bucket_name}' and the bucket itself?"
    )
    if not confirm:
