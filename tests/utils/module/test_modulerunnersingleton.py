...
import pytest
from src.utils.module.ModuleRunnerSingleton import ModuleRunnerSingleton  # Correct class import
...
def test_create_runner_fail_class_not_inheriting_AbstractRunner():
    ...
    mock_runner_class.__class__ = object  # Ensure it's just a plain class, not inheriting from AbstractRunner
    ...
    # Expecting ValueError because the class does not inherit from AbstractRunner
    # with pytest.raises(ValueError, match="Class must inherit from AbstractRunner"):
    module_runner.create_runner("pomodoro")

    with patch('importlib.import_module', return_value=False) as mock_import:
...
def test_run():
    ...
    module_runner = ModuleRunnerSingleton()
    with patch.object(module_runner, 'create_runner', return_value=MagicMock()) as mock_create_runner:
