...
from tests.shared_annotations import StandardTestCase, mock_test_module, mock_logging_context
...
class TestLoggingSetup(StandardTestCase):
    ...
    def setUp(self):
        ...
        with patch('src.utils.logging.logging_config_singleton.generate_cross_platform_logging_config', 
    ...
    @mock_test_module('logging.config', fileConfig=Mock(side_effect=FileNotFoundError("Config file doesn't exist")))
    def test_logging_setup_logging_file_dne(self):
        ...
        with pytest.raises(RuntimeError, match="Failed to load logging configuration.*Config file doesn't exist"):
    ...
    @mock_test_module('logging.config', fileConfig=Mock(side_effect=Exception("Failed to load config")))
    def test_logging_failed_config_load(self):
        ...
        with pytest.raises(RuntimeError, match="Failed to load logging configuration.*Failed to load config"):
            self.logging_setup.setup()
