...
def test_method_patcher(mock_context):
    ...
    method_behavior = lambda x: x + 1
    patcher = strategy.patch(mock_context['target_path'], 'method_name', method_behavior)
...
def test_mock_context_manager(mock_context):
    ...
    with MockContextManager(mock_context['target_path'], mock_context['method_behaviors'],
                            mock_context['attribute_values'], mock_context['mapping_values']) as context:
...
def test_update_patch(mock_context):
    ...
    with MockContextManager(mock_context['target_path'], mock_context['method_behaviors'],
                            mock_context['attribute_values']) as context:
