...
def test_update_patch_with_exception():
    ...
    with patch(
        "src.utils.test.mock_patching_strategies.MethodPatcherStrategy.execute"
    ) as mock_execute:
        ...
        with mock_context:
            ...
            mock_context.update_patch.return_value = MockUpdatePatch(
                "method_name", lambda x: x + 5
            )
            ...
            with pytest.raises(ValueError):
                with mock_context.update_patch("method_name", lambda x: x + 5):
                    ...
                    assert (
                        mock_method(2) == 7
                    )  # Should be using the side effect from MockUpdatePatch
                    raise ValueError("Test exception")
