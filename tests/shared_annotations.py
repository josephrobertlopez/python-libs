    ...
    def assert_mock_called_with_any(self, mock_obj, *expected_args):
        ...
        for args in expected_args:
            if args in call_args_list:
                return
        self.fail(f"Mock not called with any of {expected_args}. Actual calls: {call_args_list}")
...
mock_basic_sys = partial(smart_mock, "sys", argv=["program"], stdout=StringIO(), stderr=StringIO())
mock_basic_os = partial(smart_mock, "os", getcwd=lambda: "/", environ={"HOME": "/home/user"})
mock_basic_builtins = partial(smart_mock, "builtins", print=Mock(), open=lambda f, m='r': StringIO())
...
mock_test_pygame = partial(smart_mock, "pygame.mixer", init=Mock(), get_init=Mock(return_value=True))
...
__all__ = [
    ...
    'smart_mock', 'temporary_mock', 'patch_object', 'MockFactory', 'StandardTestCase',
    
    # Specialized decorators
    'mock_interactive_test', 'mock_network_test',
    'mock_test_sys', 'mock_test_os', 'mock_test_builtins', 'mock_test_module', 'output_test',
    ...
    'create_runner_test_cases', 'create_output_test_cases'
]
