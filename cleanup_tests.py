...
class TestCleaner:
    ...
    def suggest_cleanup_actions(self, patterns: Dict[str, List[Path]]) -> List[str]:
        ...
        if "excessive_mock_instances" in patterns:
            suggestions.append("🔄 Replace repeated Mock() instances with reusable fixtures in StandardTestCase")
            
        if "nested_context_candidates" in patterns:
            suggestions.append("📦 Replace nested smart_mock contexts with config_mock decorators")
            
        if "legacy_patch_decorators" in patterns:
            suggestions.append("🚀 Migrate @patch decorators to smart annotation decorators")
...
""",
    ...
    def run_cleanup_analysis(self) -> TestCleanupReport:
        ...
        print(f"   Potential line reduction: ~{estimated_reduction} lines ({estimated_reduction/total_boilerplate*100:.1f}%)")
        print(f"   Decorator patterns to add: ~{len(patterns.get('nested_context_candidates', []))}")
        print(f"   Base class migrations: ~{len(patterns.get('pytest_style_classes', []))}")
        
        self.report.patterns_replaced = len(patterns)
        self.report.decorators_added = len(patterns.get('nested_context_candidates', []))
